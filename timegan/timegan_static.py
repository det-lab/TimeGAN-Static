...
def unpack_parameters(parameters):
    ...
    return (parameters['hidden_dim'], parameters['num_layer'], parameters['iterations'], parameters['batch_size'], parameters['module'],
            parameters['dim'], 1)
...
def embedder (X, T, S, param):
    ...
    """
    hidden_dim, static_dim, num_layers, iterations, batch_size, module_name, z_dim, static_z_dim, gamma = unpack_parameters(param)
    dim = z_dim
    with tf.compat.v1.variable_scope("embedder", reuse = tf.compat.v1.AUTO_REUSE):
      ...
      # Static features
      HS = _layers.stack(S, _layers.fully_connected, [static_dim for _ in range(num_layers)], activation_fn=tf.nn.sigmoid)
...
def recovery (H, T, param):   
    ...
    with tf.compat.v1.variable_scope("recovery", reuse = tf.compat.v1.AUTO_REUSE):       
      ...
      # Static features
      rs_outputs = _layers.stack(S, _layers.fully_connected, [static_dim for _ in range(num_layers)], activation_fn=tf.nn.sigmoid)
...
def generator (Z, T, S, param):  
    ...
    with tf.compat.v1.variable_scope("generator", reuse = tf.compat.v1.AUTO_REUSE):
      ...
      # Static features
      ES = _layers.stack(S, _layers.fully_connected, [static_dim for _ in range(num_layers)], activation_fn=tf.nn.sigmoid)
    return ET, ES
...
def discriminator (H, T, S, param):
    ...
    with tf.compat.v1.variable_scope("discriminator", reuse = tf.compat.v1.AUTO_REUSE):
      ...
      c_outputs, c_last_states = tf.compat.v1.nn.biderectional_dynamic_rnn(fw_cell, bw_cell, c_inputs, dtype=tf.float32, sequence_length=T)
      ...
      ds_outputs = _layers.stack(S, _layers.fully_connected, [static_dim for _ in range(num_layers)], activation_fn=tf.nn.sigmoid)
